...
red_print "This package is use for Sensing SG6C-ORNX-G2-F on JetPack-6.1.-L4T-36.4"
...
if [ "`sudo lsmod | grep fzcam`" == "" ];then
	...
	if [ $gmslmode == 0 ];then
		...
		sudo insmod sgx-yuv-gmsl2.ko mode=0xf310f777
	fi
fi	
...
if [ $trig_mode == 1 ];then
	if [ $key == 1 ];then
		...
		v4l2-ctl -V --set-fmt-video=width=${string[0]},height=${string[1]} --set-ctrl bypass_mode=0,sensor_mode=4,trig_mode=3,trig_pin=0x67 -d /dev/video${port}
	fi
else
	if [ $key == 1 ];then
