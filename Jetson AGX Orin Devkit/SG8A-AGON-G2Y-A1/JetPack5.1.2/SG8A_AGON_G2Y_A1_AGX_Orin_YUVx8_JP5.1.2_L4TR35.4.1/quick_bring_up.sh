...
if [ -f /boot/dtb/kernel_tegra234-p3701-0000-p3737-0000.dtb ]; then
    platform=tegra234-p3701-0000-p3737-0000
	extlinux=extlinux_32g
elif [ -f /boot/dtb/kernel_tegra234-p3701-0005-p3737-0000.dtb ]; then
    platform=tegra234-p3701-0005-p3737-0000
	extlinux=extlinux_64g
else
    red_print "This device is not supported by the driver."
    exit 1
fi
...
if [ -f $PWD/camera_type ]; then
	...
	if [ ${camera_array[key]} == $var ];then
		...
		if [ "${cam_mode}" -eq 1 ] && { [ "${yuv_cam_type}" = 2 ] || [ "${yuv_cam_type}" = 4 ]; }; then
		    sudo insmod "${camera_array[key]}.ko" enable_3G_0=1,1,1,1 enable_3G_1=1,1,1,1
		else
		...
		if [ ${cam_mode} -eq 1 ];then  #For yuv_gmsl2 mode
			if [ ${yuv_cam_type} == 0 ];then
				v4l2-ctl -d /dev/video${port} -c sensor_mode=1,trig_pin=0xffff0007,trig_mode=0
			elif [ ${yuv_cam_type} == 1 ];then
				v4l2-ctl -d /dev/video${port} -c sensor_mode=1,trig_pin=0xffff0007,trig_mode=0
			elif [ ${yuv_cam_type} == 2 ];then
				v4l2-ctl -d /dev/video${port} -c sensor_mode=1,trig_pin=0xffff0007,trig_mode=0
			elif [ ${yuv_cam_type} == 3 ];then
				v4l2-ctl -d /dev/video${port} -c sensor_mode=2,trig_pin=0xffff0007,trig_mode=0
			elif [ ${yuv_cam_type} == 4 ];then
				v4l2-ctl -d /dev/video${port} -c sensor_mode=2,trig_pin=0xffff0007,trig_mode=0
			elif [ ${yuv_cam_type} == 5 ];then
				v4l2-ctl -d /dev/video${port} -c sensor_mode=3,trig_pin=0xffff0008,trig_mode=1
	...
	if [ -f /lib/modules/5.10.120-tegra/kernel/drivers/iio/imu/bmi088/bmi088.ko ]; then
                sudo rm -rf /lib/modules/5.10.120-tegra/kernel/drivers/iio/imu/bmi088/bmi088.ko
        fi
	sudo depmod
