...
static int tegra_vi_graph_notify_complete(struct v4l2_async_notifier *notifier)
{
	...
	/*
	 * Allow a camera overlay to request a stable V4L2 minor without
	 * changing sensor probe order.  Channels without the property keep
	 * the standard dynamic allocation behavior.
	 */
	if (chan->endpoint_node) {
		if (!of_property_read_u32(chan->endpoint_node,
					  "nvidia,video-minor",
					  &requested_video_nr) ||
		    (chan->endpoint_node->parent &&
		     !of_property_read_u32(chan->endpoint_node->parent,
					   "nvidia,video-minor",
					   &requested_video_nr)))
			video_nr = requested_video_nr;
	}

	ret = video_register_device(chan->video, VFL_TYPE_VIDEO, video_nr);
	if (ret < 0) {
	...
	if (video_nr >= 0) {
		if (chan->video->num != video_nr)
			dev_warn(chan->vi->dev,
				 "VI channel %u requested /dev/video%d but got /dev/video%d\n",
				 chan->id, video_nr, chan->video->num);
		else
			dev_info(chan->vi->dev,
				 "registered VI channel %u as /dev/video%d\n",
				 chan->id, chan->video->num);
	}
