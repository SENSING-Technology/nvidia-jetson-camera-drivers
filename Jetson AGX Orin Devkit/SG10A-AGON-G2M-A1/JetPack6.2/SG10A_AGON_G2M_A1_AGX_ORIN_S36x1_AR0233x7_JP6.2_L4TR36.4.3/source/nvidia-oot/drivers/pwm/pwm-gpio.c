...
static struct pwm_gpio *to_tegra_pwm_chip(struct pwm_chip *chip)
{
			  ...
			  const struct pwm_state *state)
{
    ...
    struct pwm_gpio *gpwm = to_tegra_pwm_chip(chip);
	bool invert = state->polarity == PWM_POLARITY_INVERSED;
     unsigned long flags;
...
static int pwm_gpio_probe(struct platform_device *pdev)
{
	...
	//ret = devm_pwmchip_alloc(dev, 1, sizeof(*gpwm), &chip);
	//if (IS_ERR(ret))
	//	return -ENOMEM;

	//gpwm = pwmchip_get_drvdata(chip);
    //gpwm = container_of(chip, struct pwm_gpio, chip);
    ...
    dev_info(&pdev->dev,"Init gpio to pwm success\n");
	return 0;
}
