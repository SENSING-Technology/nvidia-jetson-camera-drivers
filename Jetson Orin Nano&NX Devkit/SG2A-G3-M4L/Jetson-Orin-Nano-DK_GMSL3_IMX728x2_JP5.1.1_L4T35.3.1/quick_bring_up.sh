...
<<COMMENT
#
# 2023-07-18 Jetson Orin nano V1.0
#
COMMENT
...
csi_rate_max=$(sudo cat /sys/kernel/debug/bpmp/debug/clk/nvcsi/max_rate)
csi_rate_now=$(sudo cat /sys/kernel/debug/bpmp/debug/clk/nvcsi/rate)
...
red_print "This package is use for Jetson_Linux_R35.3.1"
camera_array=([1]=sg8-imx728c-gmsl3
               )
...
if [ -f $PWD/camera_type ]; then
	...
	if [ ${csi_rate_max} != ${csi_rate_now} ]; then
		...
		sudo cat /sys/kernel/debug/bpmp/debug/clk/emc/max_rate > /sys/kernel/debug/bpmp/debug/clk/emc/rate
	...
	if [ ${camera_array[key]} = $var ];then
		export DISPLAY=:0
		cd $PWD/ko
		sudo insmod max96792.ko
		sudo insmod max96793.ko
		sudo insmod ${camera_array[key]}.ko
		sleep 0.1
		nvgstcapture-1.0 --sensor-id=${port}
		green_print "run <nvgstcapture-1.0 --sensor-id=0> to bring up camera0"
		green_print "run <nvgstcapture-1.0 --sensor-id=1> to bring up camera1"
	else
