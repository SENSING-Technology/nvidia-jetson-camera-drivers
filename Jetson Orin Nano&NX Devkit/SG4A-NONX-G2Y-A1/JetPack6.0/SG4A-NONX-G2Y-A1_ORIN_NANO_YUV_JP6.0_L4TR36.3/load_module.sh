...
red_print "This package is use for Sensing SG4A-NONX-G2Y-A1 on JetPack-6.0-L4T-36.3.0"
...
#gst-launch-1.0 v4l2src device=/dev/video$port ! video/x-raw,format=UYVY,width=${string[0]},height=${string[1]},framerate=30/1 ! videoconvert ! ximagesink sync=false
gst-launch-1.0 v4l2src device=/dev/video$port ! "video/x-raw, format=UYVY, width=${string[0]}, height=${string[1]}, framerate=30/1" ! nvvidconv ! "video/x-raw(memory:NVMM), format=RGBA" ! nv3dsink
